...
import { ImageReSizeClass } from './ImageReSizeClass';
...
class ProjectState {
  ...
  async addProject(sendData: SendData) {
    ...
    if (sendData.id === undefined || sendData.id === '') {
      ...
      if (sendData.imgFile) {
        const inputImgFile = sendData.imgFile;
        ...
        const inputImgFile2: File| undefined| void = await resizeClass.resizeImage(inputImgFile, 400);
        // console.log('resize => inputImgFile2:', inputImgFile2);

        if (inputImgFile2 !== undefined) {
          //画像ファイルのアップロード
          await firebaseStorage.ref().child("image/" + sendData.id + imgsoezi).put(inputImgFile2, metadata)
            .then((docRef) => {
              console.log("addProject Document ImgSet ID: ", sendData.id);
  ...
  async dbset(project: Project, inputImgFile ? : File) {
    ...
    if (inputImgFile) {
      ...
      if (inputImgFile2 !== undefined) {
        //画像ファイルのアップロード
        await firebaseStorage.ref().child("image/" + newId + imgsoezi).put(inputImgFile2, metadata)
          .then((docRef) => {
            console.log("dbset ImgSet ID: ", newId);
            console.log('dbset uploaddocRef:', docRef);
          })
          .catch((error) => {
            console.error("Error dbset ImgSet: ", error);
          });
      }
    }
  }
...
class ProjectInput {
  ...
  submitHandler(event: Event) {
    ...
    console.log('ProjectInput inputImgFile:', inputImgFile);
